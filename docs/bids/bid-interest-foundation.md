...
웹 수동 수집은 기본 7일이며 2·7·14·30일 중 선택할 수 있다. API 페이지는 허용
범위 내에서 최대 999건씩 받고, 세 보조 작업은 병렬 조회한다. 목록은 응답 지연을
막기 위해 검색 조건별 마감순 최대 200건을 표시한다.

지역·업종 표시 문자열은 보조 응답의 명칭 필드만 사용한다. 제한 일련번호나 Y/N
플래그처럼 필드명에 `lmt`가 들어간 비명칭 값은 화면과 매칭 텍스트에 섞지 않는다.
