...
class AwsIamInlineTenantSecretGrantReadOnlyVerifier:
    ...
    inline policy names. The expected resource pattern must be bound to the exact
    planned tenant ``secret_id`` before any provider read can occur. This adapter
    then performs only ``GetRolePolicy`` and validates that the returned document
    is the exact single-secret policy defined by the runtime policy contract.

    It never creates, attaches, updates, detaches, or deletes a policy and never
    constructs an AWS session or credential source. ``NoSuchEntity`` is a
    definitive not-ready result. Permission, transport, throttling, malformed
    response, or any other ambiguous failure propagates or fails closed rather
    ...
    def __init__(
        ...
        *,
        ...
        self._secret_resource_pattern = normalize_exact_tenant_secret_resource_pattern(
            secret_id=secret_id,
