...
def ensure_admin_schema(cur):
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS gis.definition_layer_group (
          id uuid PRIMARY KEY,
          group_code text NOT NULL UNIQUE CHECK(group_code ~ '^[a-z_][a-z0-9_]*    cur.execute("""
...
def admin_schema_ready(cur):
    cur.execute("""
      SELECT
        to_regclass('gis.definition_layer_group') IS NOT NULL
...
def layer_layer_group_state(cur, group_id):
...
def business_layer_group_state(cur, group_id):
...
def impact_for_group(cur, group_id):
    ...
    return {"group_layers": layer_count, "group_fields": 0}
