...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunSQL(
            ...
            -- Stage nodes are direct children of event.stage; their children are
            -- the dependent Event Types. No duplicated event.type.<stage> layer.
            INSERT INTO ops.settings_nodes
            ...
            SELECT category.id, seed.code, seed.name, 'category', seed.system_key, seed.ord, true
              ...
              CROSS JOIN (VALUES
                  ...
                  ('settlement', '정산', 'workflow.stage.settlement', 70)
              ) seed(code, name, system_key, ord)
             WHERE category.field_ref = 'event.stage';

            INSERT INTO ops.settings_nodes
                (parent_id, code, name, node_type, system_key, ord, locked)
            SELECT stage.id, seed.code, seed.name, 'value', seed.system_key, seed.ord, true
              FROM ops.settings_nodes stage
              JOIN (VALUES
                  ('workflow.stage.preparation', 'estimate', '견적', 'workflow.event.estimate', 10),
                  ('workflow.stage.preparation', 'bid', '입찰', 'workflow.event.bid', 20),
                  ('workflow.stage.preparation', 'award', '낙찰', 'workflow.event.award', 30),
                  ('workflow.stage.contract', 'contract_sign', '체결', 'workflow.event.contract_sign', 10),
                  ('workflow.stage.contract', 'contract_change', '변경', 'workflow.event.contract_change', 20),
                  ('workflow.stage.contract', 'contract_cancel', '취소', 'workflow.event.contract_cancel', 30),
                  ('workflow.stage.kickoff', 'kickoff_doc', '착수계', 'workflow.event.kickoff_doc', 10),
                  ('workflow.stage.kickoff', 'kickoff_meeting', '착수회의', 'workflow.event.kickoff_meeting', 20),
                  ('workflow.stage.kickoff', 'kickoff_approval', '착수승인', 'workflow.event.kickoff_approval', 30),
                  ('workflow.stage.execution', 'work_report', '업무보고', 'workflow.event.work_report', 10),
                  ('workflow.stage.execution', 'suspend', '중지', 'workflow.event.suspend', 20),
                  ('workflow.stage.execution', 'resume', '재개', 'workflow.event.resume', 30),
                  ('workflow.stage.closeout', 'completion_doc', '준공계', 'workflow.event.completion_doc', 10),
                  ('workflow.stage.closeout', 'completion_inspection', '준공검사', 'workflow.event.completion_inspection', 20),
                  ('workflow.stage.closeout', 'completion_approval', '준공승인', 'workflow.event.completion_approval', 30),
                  ('workflow.stage.settlement', 'advance_payment', '선급금', 'workflow.event.advance_payment', 10),
                  ('workflow.stage.settlement', 'progress_payment', '기성금', 'workflow.event.progress_payment', 20),
                  ('workflow.stage.settlement', 'final_payment', '준공금', 'workflow.event.final_payment', 30)
              ) seed(stage_key, code, name, system_key, ord)
                ON stage.system_key = seed.stage_key;
