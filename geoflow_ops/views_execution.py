...
from .models import ProcessEvent, Project
from .services.department_routing import route_project_inspection_request_to_management
...
def _employee_options(alias: str):
    with connections[alias].cursor() as cur:
        ...
        return [
            {"id": row[0], "name": row[1] or "", "title": row[2] or "", "department_id": row[3] or ""}
            for row in cur.fetchall()
        ]


def _is_inspection_request_task(row: dict[str, Any]) -> bool:
    tokens = {
        str(row.get("l2_code") or "").strip().lower(),
        str(row.get("l3_code") or "").strip().lower(),
        str(row.get("l2_name") or "").strip().replace(" ", "").lower(),
        str(row.get("l3_name") or "").strip().replace(" ", "").lower(),
    }
    return bool({"inspection_request", "검사요청"}.intersection(tokens))


def _create_inspection_handoff_event(alias: str, request, project: Project, task_row: dict[str, Any]) -> None:
    source_item_id = str(task_row["item_id"])
    ...
    management_department = route_project_inspection_request_to_management(alias, project.pk)
    actor = (
        getattr(request.user, "username", None)
        or getattr(request.user, "email", None)
        or "unknown"
    )
    ProcessEvent.objects.using(alias).create(
        scope_type="project",
        scope_id=project.pk,
        ...
        payload={
            "source": "project_scope_item",
            "source_scope_item_id": source_item_id,
            "handoff": "business_to_management",
            "autogenerated": True,
        },
        created_by=actor,
    )
...
def project_task_save(request, pk):
    ...
    can_assign = gf_has_perm(request, "directory.view")
    task_metadata = {row["item_id"]: row for row in _load_task_rows(alias, project.pk)}
    ...
    with connections[alias].cursor() as cur:
        cur.execute(
            """
            SELECT id::text, design_qty, assignee_employee_id::text, status
              FROM prj.scope_item
        ...
        existing = {
            row[0]: {"design_qty": row[1], "assignee_employee_id": row[2] or "", "status": row[3] or "pending"}
            for row in cur.fetchall()
        }
    ...
    for item_id, data in submitted.items():
        ...
        validation_error = _completion_validation(status, existing[item_id]["design_qty"], completed_qty, variance_reason)
        if validation_error:
        ...
        prepared.append(
            {
                "id": item_id,
                "status": status,
                "previous_status": existing[item_id]["status"],
            ...
            for row in prepared:
                cur.execute(
                    ...
                    UPDATE prj.scope_item
                       SET status=%s, progress_qty=%s, completed_qty=%s,
                           completed_at=%s, assignee_employee_id=%s,
                           variance_reason=%s, remark=%s, updated_at=now()
                     WHERE id=%s AND project_id=%s
                    """,
                    [row["status"], row["progress_qty"], row["completed_qty"], row["completed_at"],
                     row["assignee_employee_id"], row["variance_reason"], row["remark"], row["id"], str(project.pk)],
                )
                if cur.rowcount != 1:
                    raise RuntimeError("project task update lost scope")

        for row in prepared:
            metadata = task_metadata.get(row["id"])
            if (
                metadata
                and row["previous_status"] != "done"
                and row["status"] == "done"
                and _is_inspection_request_task(metadata)
            ):
                _create_inspection_handoff_event(alias, request, project, metadata)
