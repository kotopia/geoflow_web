...
def _table_exists(alias: str|None,relation:str)->bool:
    ...
    try:
        with connections[alias].cursor() as cur:
            cur.execute("SELECT to_regclass(%s) IS NOT NULL",[relation]);row=cur.fetchone()
        return bool(row and row[0])
    except Exception:return False

def normalize_contract_status(value:object)->str:
    text=str(value or "").strip();return CONTRACT_STATUS_ALIASES.get(text.lower(),CONTRACT_STATUS_ALIASES.get(text,text))

def _fallback_for(system_key:str):
    if system_key=="contract.status":return CONTRACT_STATUS_FALLBACK
    if system_key=="contract.kind":return CONTRACT_KIND_FALLBACK
    if system_key=="hr.employment_type":return EMPLOYMENT_TYPE_FALLBACK
    if system_key=="event.stage":return tuple((choice.code,choice.label) for choice in STAGE_CHOICES)
    if system_key=="event.status":return tuple((choice.code,choice.label) for choice in EVENT_STATUS_CHOICES)
    if system_key.startswith("event.type."):
        group=system_key.rsplit(".",1)[-1]
        return tuple((event_type,_EVENT_LABELS.get(event_type,event_type)) for event_type,default_group in EVENT_DEFAULT_STAGE.items() if default_group==group)
    return ()
            ...
            cur.execute("""
                SELECT child.code, child.name, child.active
                 ...
                 WHERE category.active = true
                   AND (category.system_key = %s OR (category.system_key IS NULL AND category.code = %s AND category.parent_id = (SELECT id FROM ops.settings_nodes WHERE system_key='event.type' LIMIT 1)))
                 ORDER BY child.ord, child.name, child.code
            """,[system_key,group])
        else:
            cur.execute("""
                SELECT child.code, child.name, child.active
                  FROM ops.settings_nodes category JOIN ops.settings_nodes child ON child.parent_id=category.id
                 WHERE category.system_key=%s AND category.active=true
                 ORDER BY child.ord, child.name, child.code
            """,[system_key])
        return cur.fetchall()
...
def _merge_required_options(system_key:str,options):
    ...
    if not required:return list(options)
    required_codes={str(code or "") for code,_label in required};result=list(required)
...
def settings_options(alias:str|None,system_key:str,*,include_inactive:bool=False):
    ...
    if not alias or not _table_exists(alias,"ops.settings_nodes"):return _merge_required_options(system_key,fallback)
    try:rows=_configured_rows(alias,system_key)
    except Exception:return _merge_required_options(system_key,fallback)
    if not rows:return _merge_required_options(system_key,fallback)
...
def event_workflow_options(alias:str|None):
    ...
    required_stage_codes={choice.code for choice in STAGE_CHOICES};process_stages=[row for row in process_stages if row[0] in required_stage_codes]
    categories=list(process_stages);categories.append(("settlement","정산"));statuses=settings_options(alias,"event.status");types_by_stage={}
...
def event_type_allowed(alias:str|None,stage:str,event_type:str)->bool:
    stage=str(stage or "").strip();event_type=str(event_type or "").strip()
    if not stage or not event_type:return False
    if stage=="settlement":return event_type in settings_codes(alias,"event.type.settlement")
