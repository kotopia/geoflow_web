(function () {
  ...
  function run() {
    var match = window.location.pathname.match(/\/contracts\/([0-9a-f-]{36})\/?$/i);
    ...
    if (!heading) return;
    fetch('/contracts/' + match[1] + '/finance-summary/', { credentials: 'same-origin', headers: { 'X-Requested-With': 'XMLHttpRequest' } })
      ...
      .then(function (d) {
        ...
        card.innerHTML = '<div class="card-header d-flex align-items-center justify-content-between"><h5 class="card-title mb-0">Finance 요약</h5><a class="btn btn-sm btn-outline-primary" href="/finance/">Finance 열기</a></div>' +
