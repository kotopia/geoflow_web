(function () {
  ...
  function popupNode(feature, layerInfo) {
    ...
    var rows = [
      ["UUID", feature.id || properties.id], ["시설물 코드", properties.ftr_cde],
      ["외부 시설물 ID", properties.ftr_idn], ["소스 키", properties.source_key],
      ["소스 유형", properties.source_type], ["측량 코드", properties.survey_code || properties.code],
      ["측량일", properties.survey_date], ["명칭", properties.name],
    ...
    rows.forEach(function (row) {
      ...
      dt.textContent = row[0]; dd.textContent = String(row[1]);
  ...
  function initProjectMap() {
    ...
    function createState(layerInfo) {
      var state = { info: layerInfo, group: null, featureLayers: {}, returned: 0, truncated: false };
    ...
    function clearState(state) { state.group.clearLayers(); state.featureLayers = {}; state.returned = 0; }
    ...
    async function loadLayer(state, bbox) {
      ...
      var response = await fetch(endpoint + "?" + params.toString(), { credentials: "same-origin", headers: { "Accept": "application/json" } });
      if (!response.ok) throw new Error(state.info.standard_name + " HTTP " + response.status);
      var data = await response.json();
      clearState(state); state.group.addData(data);
      state.returned = state.group.getLayers().length;
    ...
    async function refreshChangedLayer(state, objectIds, currentRevision) {
      ...
      var params = new URLSearchParams({ layer: state.info.standard_name, ids: objectIds.join(","), bbox: bboxString(map) });
      var response = await fetch(featureEndpoint + "?" + params.toString(), { credentials: "same-origin", headers: { "Accept": "application/json" } });
    ...
    async function applyRealtimeEvent(payload) {
      ...
      (payload.changes || []).forEach(function (change) {
        ...
        if (!id) return;
        if (change.action === "delete") { removeFeature(state, id); return; }
      ...
      var tasks = Object.keys(grouped).map(function (layerName) {
        return refreshChangedLayer(layerState[layerName], Array.from(new Set(grouped[layerName])), payload.current_revision || 0);
      });
    ...
    function connectWebSocket() {
      ...
      socket.onclose = function (event) {
        ...
        if (event && (event.code === 4400 || event.code === 4401 || event.code === 4403)) {
          websocketStarted = false;
          if (reconnectTimer) { clearTimeout(reconnectTimer); reconnectTimer = null; }
          setStatus(event.code === 4401 ? "실시간 연결 인증이 만료되었습니다." : "실시간 연결 권한을 확인할 수 없습니다.");
          return;
        }
    ...
    loadAll({ fit: true }).then(function () {
      ...
      map.on("moveend", function () {
        ...
        debounceTimer = setTimeout(function () {
          loadAll({ bbox: bboxString(map), fit: false }).catch(function () { setStatus("현재 지도 영역을 다시 불러오지 못했습니다."); });
        }, 250);
      });
