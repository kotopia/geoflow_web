...
class QFieldSyncConcurrencyContractTests(SimpleTestCase):
    ...
    def test_qfield_sidecar_uses_declarative_edit_signals_and_feature_form_fallback(self):
        ...
        self.assertIn("Instantiator", text)
        self.assertIn("Connections", text)
        self.assertIn("function onGeometryChanged(fid, geometry)", text)
        self.assertIn("function onAttributeValueChanged(fid, index, value)", text)
        self.assertIn("function onFeatureAdded(fid)", text)
        self.assertIn("function onFeatureDeleted(fid)", text)
        self.assertIn('iface.findItemByObjectName("featureForm")', text)
        self.assertIn("function onStateChanged()", text)
        self.assertIn("captureFocusedFeature", text)
        self.assertIn("resolveObjectId", text)
        self.assertIn("refreshLayerFidMap", text)
        self.assertIn("editAcceptedSyncTimer", text)
        self.assertIn("captureSuppressed = true", text)
        self.assertIn("managedLayerDescriptors = plan.layers || []", text)
        self.assertIn("qgisProject.mapLayersByName", text)
        self.assertIn("xhr.status === 401", text)
        self.assertIn("xhr.status === 403", text)
        self.assertIn("GeoFlow Field 1.0", text)
        self.assertNotIn(".geometryChanged.connect", text)
        self.assertNotIn(".attributeValueChanged.connect", text)
