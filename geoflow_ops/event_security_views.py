...
from .services.tenant_settings import event_type_allowed, event_workflow_options, settings_codes


ASSIGNMENT_FIELDS = {"owner_department_id", "assignee_employee_id"}
EVENT_ONLY_CATEGORIES = {"settlement"}
...
def _valid_event_category(alias: str, category: str) -> bool:
    return category in EVENT_ONLY_CATEGORIES or category in settings_codes(alias, "event.stage")
...
def _workflow_error(alias: str, data: dict, *, existing=None, creating: bool):
    if creating:
        ...
        if not _valid_event_category(alias, stage):
    ...
    incoming_type = normalize_event_type_for_write(data.get("event_type")) if "event_type" in data else existing_type
    ...
    if stage_changed or type_changed:
        if not _valid_event_category(alias, incoming_stage):
            return "Invalid stage"
...
def workflow_options(request):
    ...
    return JsonResponse({
        "stages": [{"code": code, "label": label} for code, label in options["stages"]],
        "process_stages": [{"code": code, "label": label} for code, label in options.get("process_stages", [])],
        "statuses": [{"code": code, "label": label} for code, label in options["statuses"]],
        "types_by_stage": {
            stage: [{"code": code, "label": label} for code, label in rows]
...
def event_update(request, event_id):
    ...
    if data is not None:
        ...
        error = _workflow_error(alias, data, existing=existing, creating=False)
        if error:
            return JsonResponse({"error": error}, status=400)
        _replace_request_json_body(request, data)
